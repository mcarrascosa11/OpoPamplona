...
const adaptiveTestPlugin = () => ({
  ...
  transform(code, id) {
    ...
    if (!code.includes('seleccionarPreguntasAdaptativas(base')) {
      ...
      code = code.replace(selectionMarker, 'const seleccion = (modo === "aprendido" ? seleccionarPreguntasAdaptativas(base, state.preguntas || {}, Math.min(num, base.length)) : shuffle(base).slice(0, Math.min(num, base.length))).map((q) => ({ ...q, _order: shuffle([0, 1, 2, 3]) }));');
    }
...
export default defineConfig({ plugins: [react(), adaptiveTestPlugin()] });
